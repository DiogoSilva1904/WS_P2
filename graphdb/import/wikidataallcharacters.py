...
class WikidataToRDF:
    ...
    def get_all_star_wars_characters(self, limit=500):
        #query filters chracerts by star wars that are not droids(and any subclasses of it)
        query = f"""
        SELECT ?character ?characterLabel WHERE {{
            ?character wdt:P31 wd:Q33125444 .  # instance of fictional character
            
            FILTER NOT EXISTS {{
            ?character wdt:P31 ?type .
            ?type wdt:P279* wd:Q52383  # excludes droids and subclasses of droids
            }}
        ...
        try:
            ...
            print(f"Error retrieving Star Wars characters: {e}")
            return []
    ...
    def generate_all_star_wars_characters_rdf(self, limit=100):
        chars = self.get_all_star_wars_characters(limit=limit)
        print(f"Found {len(chars)} characters")

        all_predicates = set()

        for qid, label in chars:
        ...
        # 🔁 Get and annotate predicate labels
        ...
        # 💾 Serialize RDF graph
        self.graph.serialize("starwars_characters.ttl", format="xml")
        print(f"RDF graph with {len(self.graph)} triples saved to starwars_characters.ttl")
...
wikidata_rdf.generate_all_star_wars_characters_rdf(limit=10)
